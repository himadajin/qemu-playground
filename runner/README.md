...
- ベース: `ubuntu:24.04`(linux/arm64 前提)
- パッケージ(apt、`--no-install-recommends`):
  - RV64: `gcc-riscv64-linux-gnu`、`binutils-riscv64-linux-gnu`、`libc6-dev-riscv64-cross`
  - AArch64(ネイティブ): `gcc`、`binutils`、`libc6-dev`(triplet 付き `aarch64-linux-gnu-*` コマンドが提供される)
...
`gcc` は `libc6-dev` / `libc6-dev-riscv64-cross` を Recommends としてのみ要求するため、
...
AArch64 はホストのネイティブアーキテクチャのため `/usr/aarch64-linux-gnu` のような sysroot は存在せず、
動的リンカはコンテナのルート(`/lib/ld-linux-aarch64.so.1`)にある。`-L` 指定は不要(付けるなら `-L /`)。
